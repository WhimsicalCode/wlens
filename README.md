...
Beyond dbt models and sources, wlens supports custom entity types declared
in `wlens.yml` — product events, feature flags, customer attributes, etc.
The reference implementation is `events`: a YAML catalog of analytics
events inlined as `### <event-name>` sections in a target dbt source file.

See [`docs/custom-entities.md`](docs/custom-entities.md).
